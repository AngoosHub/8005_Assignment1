...
Basic.py
    ...
    Implements no multithreading or multiprocessing.
...
def read_user_input():
    ...
    print("Starting Basic program.")

    try:
        keep_running = True
        while keep_running:
            print('--------------------------------------\n'
                  'Basic Implementation. No multiple threads or processes.\n'
                  'Enter the number of tasks to complete.\n'
                  '(Input must be an integer from 0 to 50 inclusive).\n'
                  'Commands:\n'
            ...
            try:
                ...
                print('Please enter an integer from 0 to 50')
                continue
...
def basic_loop(total_tasks):
    """
    Performs the mathematical and I/O tasks with simple for loop, no implementations of
    ...
    for task_counter in range(total_tasks):
        ...
        number = int_list[task_counter]
        factor_list = prime_factorization(number)
        save_data(number, factor_list)

        loop_time = time.perf_counter() - time_loop_start
        loop_time_list.append(loop_time)
        print(f"Task {task_counter+1} time: {loop_time}\n")

    total_time = time.perf_counter() - time_total_start
    avg_task_time = sum(loop_time_list) / len(loop_time_list)
    ...
    try:
        with open(file=LOG_PATH, mode="a", encoding='utf8') as file:
            ...
            for idx, task_time in enumerate(loop_time_list):
                data_log += f"Task {idx+1} time: {task_time}\n"
